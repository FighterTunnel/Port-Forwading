...
for IFACE in $ACTIVE_INTERFACES; do
    ...
    IFACE_IP=$(ip -4 addr show "$IFACE" | grep -oP '(?<=inet\s)\d+\.\d+\.\d+\.\d+' | head -n 1 || echo "")
    ...
    # Tambahkan rule policy routing (berdasarkan source IP)
    if [ -n "$IFACE_IP" ]; then
        echo "Membuat rule routing dari source IP $IFACE_IP ($IFACE) ke tabel $TABLE_NAME priority $PRIORITY..."
        # Hapus rule lama berdasarkan oif jika ada (migrasi)
        sudo ip rule del oif "$IFACE" priority $PRIORITY 2>/dev/null || true
        # Tambahkan rule baru berdasarkan source IP
        sudo ip rule add from "$IFACE_IP" lookup "$TABLE_NAME" priority $PRIORITY
    else
        echo "WARN: Tidak dapat menemukan IP address untuk $IFACE. Rule 'from' tidak dapat ditambahkan."
        # Fallback ke OIF jika tidak ada IP? Atau skip? Untuk sekarang, kita skip.
        # echo "Fallback membuat rule routing untuk $IFACE (oif) ke tabel $TABLE_NAME priority $PRIORITY..."
...
# cat /etc/resolv.conf
echo -e "\nKonfigurasi DNS (/etc/resolv.conf akan dikelola oleh NetworkManager berdasarkan koneksi aktif):"
...
for IFACE in $ACTIVE_INTERFACES; do
    ...
    CONN_NAME=$(nmcli -t -f NAME,DEVICE con show --active | grep ":$IFACE$" | cut -d: -f1 | head -n 1)
     ...
     if [ -n "$CONN_NAME" ]; then
        ...
        echo "  IPv4 DNS: $DNS4_SET (Ignore Auto: $IGNORE_AUTO4)" # Seharusnya kosong sekarang
        echo "  IPv6 DNS: $DNS6_SET (Ignore Auto: $IGNORE_AUTO6)" # Seharusnya kosong sekarang
     else
